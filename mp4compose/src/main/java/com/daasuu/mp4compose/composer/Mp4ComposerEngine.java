...
class Mp4ComposerEngine {
    ...
    private void runPipelines() {
        ...
        while (!canceled && !(videoComposer.isFinished() && audioComposer.isFinished())) {
            ...
            if (durationUs > 0 && loopCount % PROGRESS_INTERVAL_STEPS == 0) {
                long presentationVideoTimeUs = videoComposer.getWrittenPresentationTimeUs();
                if (progressCallback != null) {
                    progressCallback.onCurrentVideoTime(presentationVideoTimeUs);
                }
                double videoProgress = videoComposer.isFinished() ? 1.0 : Math.min(1.0, (double) presentationVideoTimeUs / durationUs);
    ...
    interface ProgressCallback {
         ...
         *
         * @param timeUs Current timeUs, in Us
         */
        void onCurrentVideoTime(long timeUs);
    }
}
